...
# silently reuses the stale copy. Read from packages.tsv so a package added to the build is cleared
# here too.
...
if ! grep -q "DATADOG_E2E_DONE PASS" "${LOG_FILE}"; then
    ...
    echo "::error::Datadog emulator checks failed or timed out"
    exit 1
fi
