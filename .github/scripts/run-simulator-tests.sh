...
# The app's own intermediate output has to go too, not just the NuGet cache. The Datadog iOS
# packages' native payload is extracted out of the package into obj/ and copied into the .app, and
# neither step re-runs when the package version string is unchanged - so a rebuilt package of the
# same version leaves the *previous* build's xcframeworks embedded in the app.
rm -rf "${REPO_ROOT}/tests/DatadogNet.DeviceTests/obj" \
...
# device names exist depends on the installed Xcode, and pinning one couples this script to a runner
# image that changes without notice. Newest runtime first, so the picked device is the most current
# one available.
#
...
if ! grep -q "DATADOG_E2E_DONE PASS" "${LOG_FILE}"; then
    ...
    xcrun simctl spawn "${udid}" log show --last 2m --predicate "process == 'DatadogNet.DeviceTests'" \
        2>/dev/null | tail -100 | tee -a "${LOG_FILE}" || true
    echo "::error::Datadog simulator checks failed or timed out"
    exit 1
fi
