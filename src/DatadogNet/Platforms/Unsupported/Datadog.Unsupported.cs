...
/// </summary>
/// <remarks>
/// Reached by a MAUI app's Windows or Mac Catalyst head, and by a unit test running against the
/// library outside a platform head. Every member is a no-op so that shared code needs no
/// conditionals: a view-model that reports a RUM view runs unchanged in a test, and a Windows head
/// links without a single <c>#if</c>.
/// <para>
...
/// </para>
/// <para>
/// Mac Catalyst is here rather than mapped onto the iOS implementation on purpose. dd-sdk-ios does
/// ship a macCatalyst slice, but <c>DatadogNet.iOS</c> strips the archive to the two iOS slices —
/// the packages are <c>net*-ios</c> bindings, and a <c>net*-maccatalyst</c> project cannot
/// reference them.
/// </para>
/// </remarks>
